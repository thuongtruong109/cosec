...
export default function TopNav({
  ...
  return (
    <header className="h-14 border-b border-zinc-800/80 bg-zinc-950/95 backdrop-blur-xl px-4 sm:px-6 flex items-center justify-between sticky top-0 z-40 select-none">
        ...
        >
          ...
          <div>
            <div className="flex items-center space-x-1.5">
              ...
              <span className="bg-indigo-500/20 text-indigo-300 text-[10px] font-bold px-1.5 py-0.5 rounded-md border border-indigo-500/30 font-mono">
                AI
              </span>
            </div>
          </div>
          ...
          >
            ...
            <ChevronDown
              ...
              className={`text-zinc-400 transition-transform duration-200 ${
                showProjectDropdown ? 'rotate-180 text-indigo-400' : ''
              }`}
            />
          </button>
          ...
          <AnimatePresence>
            {showProjectDropdown && (
              <motion.div
                ...
                className="absolute left-0 mt-2 w-72 bg-zinc-900/95 backdrop-blur-xl border border-zinc-700/80 rounded-2xl shadow-2xl p-3 z-50 text-xs text-zinc-300 divide-y divide-zinc-800/80"
              >
                <div className="pb-2">
      ...
      <div className="relative hidden md:block w-80 lg:w-96" ref={searchRef}>
        <div className="relative flex items-center">
          ...
          <input
            ...
            className="w-full bg-zinc-900/90 border border-zinc-800 hover:border-zinc-700 focus:border-indigo-500 rounded-xl pl-9 pr-8 py-1.5 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-mono"
          />
          {searchQuery && (
            <button
              onClick={() => {
        ...
        <AnimatePresence>
          {showSearchDropdown && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -6, scale: 0.98 }}
              transition={{ duration: 0.15 }}
              className="absolute left-0 right-0 mt-2 bg-zinc-900/95 backdrop-blur-xl border border-zinc-700/80 rounded-2xl shadow-2xl overflow-hidden z-50 max-h-96 overflow-y-auto divide-y divide-zinc-800"
            >
              {searchIssueResults.length > 0 && (
                <div>
                  <div className="px-3.5 py-2 bg-zinc-950/80 text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono">
                  ...
                  {searchIssueResults.map((iss) => (
                    <div
                      ...
                      className="p-3 hover:bg-zinc-800/70 cursor-pointer border-b border-zinc-800/40 flex items-start space-x-2.5 transition-colors"
                    >
                      <AlertTriangle size={14} className="text-amber-400 mt-0.5 shrink-0" />
                      <div className="min-w-0">
                        <div className="text-xs text-zinc-200 font-semibold truncate">{iss.title}</div>
                ...
                <div>
                  ...
                  {searchFileResults.map((f) => (
                    <div
                      ...
                      className="p-3 hover:bg-zinc-800/70 cursor-pointer border-b border-zinc-800/40 flex items-center space-x-2.5 transition-colors"
                    >
              ...
              {searchFileResults.length === 0 && searchIssueResults.length === 0 && (
                <div className="p-5 text-center text-xs text-zinc-400 font-mono">
      ...
      <div className="flex items-center space-x-2.5 sm:space-x-3">
        ...
        <div className="hidden sm:flex items-center space-x-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[11px] font-medium font-mono shadow-sm">
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className="p-2 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-zinc-400 hover:text-white transition-colors relative cursor-pointer"
          >
            <Bell size={16} />
            {analysis && (
              <span className="absolute 1 top-1.5 right-1.5 w-2 h-2 bg-indigo-500 rounded-full ring-2 ring-zinc-950" />
            )}
          </button>
          ...
          <AnimatePresence>
            {showNotifications && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -6, scale: 0.98 }}
                transition={{ duration: 0.15 }}
                className="absolute right-0 mt-2 w-80 bg-zinc-900/95 backdrop-blur-xl border border-zinc-700/80 rounded-2xl shadow-2xl p-4 z-50"
              >
                ...
                {analysis ? (
                  <div className="space-y-2.5 text-xs">
                    <div className="p-3 rounded-xl bg-zinc-950/80 border border-zinc-800 flex items-start space-x-2.5">
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-zinc-400 mt-1 leading-relaxed">
                          Identified {analysis.issues.length} findings. Repository health score: <span className="text-indigo-400 font-bold">{analysis.scores.overall}/100</span>.
                        </div>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="text-xs text-zinc-400 py-4 text-center font-mono">
