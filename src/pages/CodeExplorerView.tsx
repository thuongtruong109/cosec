  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] bg-zinc-950 text-zinc-200 select-none overflow-hidden">
      ...
      <div className="w-64 border-r border-zinc-800 bg-zinc-950 flex flex-col shrink-0">
        <div className="p-3 border-b border-zinc-800 text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono flex items-center justify-between">
          <span>Repository Explorer</span>
          <span className="text-zinc-600">{project.files.length} files</span>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-2 truncate">
                  <FileCode size={14} className={isSelected ? 'text-indigo-400' : 'text-zinc-500'} />
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-zinc-900/60">
        <div className="h-10 border-b border-zinc-800 bg-zinc-950 px-4 flex items-center justify-between">
          <div className="flex items-center space-x-2 font-mono text-xs text-zinc-300">
            ...
            <span className="text-zinc-600">({selectedFileItem?.lines} lines)</span>
          </div>

          {fileIssues.length > 0 && (
            <div className="flex items-center space-x-1.5 px-2.5 py-1 rounded bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs font-mono font-semibold">
            ...
            return (
              <div
                ...
                className={`flex items-start group rounded ${
                  lineIssue
                    ? isHighlighted
                      ? 'bg-rose-500/20 border-l-2 border-rose-500 text-rose-200 font-semibold'
                      : 'bg-rose-500/10 hover:bg-rose-500/15 cursor-pointer'
              ...
              >
                ...
                <pre className="flex-1 whitespace-pre-wrap break-all text-zinc-200">{lineText}</pre>
      ...
      <div className="w-96 border-l border-zinc-800 bg-zinc-950 p-5 overflow-y-auto shrink-0 space-y-6">
        <div className="flex items-center justify-between pb-3 border-b border-zinc-800">
          <div className="flex items-center space-x-2">
            <Sparkles size={18} className="text-indigo-400" />
            <h3 className="font-bold text-white text-sm">AI Code Review Panel</h3>
          </div>
          ...
          <div className="space-y-5 text-xs">
            ...
            <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2">
              <div className="flex items-center justify-between">
                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-rose-500/20 text-rose-400 border border-rose-500/30 uppercase font-mono">
            ...
            <div className="space-y-1">
              <div className="text-[10px] font-bold text-zinc-500 uppercase font-mono">Description</div>
              <p className="text-zinc-300 leading-relaxed">{selectedIssue.description}</p>
            </div>

            {/* Impact & Exploitation */}
            <div className="space-y-1">
              <div className="text-[10px] font-bold text-rose-400 uppercase font-mono">Potential Impact</div>
              <p className="text-rose-200/90 leading-relaxed bg-rose-950/20 p-2.5 rounded-lg border border-rose-500/20">
            ...
            {selectedIssue.exploitationScenario && (
              <div className="space-y-1">
                <div className="text-[10px] font-bold text-amber-400 uppercase font-mono">Exploitation Scenario</div>
                <p className="text-amber-200/90 leading-relaxed bg-amber-950/20 p-2.5 rounded-lg border border-amber-500/20">
                  {selectedIssue.exploitationScenario}
                </p>
              </div>
            )}
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button
                ...
                className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-lg flex items-center justify-center space-x-2 transition-colors shadow-lg cursor-pointer"
              >
                <Check size={14} />
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-1.5 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 border border-zinc-800 rounded-lg flex items-center justify-center space-x-1.5 text-[11px] transition-colors"
                >
                  <Copy size={12} />
                  <span>Copy Fix</span>
                </button>
                ...
                <button
                  ...
                  className="flex-1 py-1.5 bg-zinc-900 hover:bg-zinc-800 text-zinc-400 border border-zinc-800 rounded-lg flex items-center justify-center space-x-1.5 text-[11px] transition-colors"
                >
                  <EyeOff size={12} />
