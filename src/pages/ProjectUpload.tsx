...
export default function ProjectUpload({ onProjectLoaded, onStartDemo }: ProjectUploadProps) {
  ...
  return (
    <div className="p-5 max-w-5xl mx-auto space-y-6 select-none font-sans">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-200 dark:border-zinc-800/80">
        ...
        <button
          ...
          className="px-3.5 py-2 bg-indigo-50 dark:bg-indigo-600/20 hover:bg-indigo-100 dark:hover:bg-indigo-600/30 border border-indigo-200 dark:border-indigo-500/30 text-indigo-700 dark:text-indigo-300 text-xs font-semibold rounded-xl flex items-center space-x-2 transition-colors shrink-0 cursor-pointer shadow-sm"
        >
      ...
      {!stagedProject ? (
        <div className="space-y-5">
          {activeTab === 'url' && (
            <div className="bg-white dark:bg-zinc-900/70 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 space-y-4 shadow-sm">
              <div>
                <label className="text-xs font-semibold text-slate-700 dark:text-zinc-300 font-mono flex items-center gap-1.5 mb-1.5">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-zinc-100 font-mono focus:outline-none focus:border-indigo-500"
                  />
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={handlePasteGitUrl}
                      className="px-3.5 py-2 bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 active:bg-slate-300 text-slate-700 dark:text-zinc-200 hover:text-slate-900 dark:hover:text-white font-medium text-xs rounded-xl transition-colors flex items-center justify-center space-x-1.5 shrink-0 cursor-pointer border border-slate-200 dark:border-zinc-700 shadow-sm"
                      title="Paste URL from Clipboard"
                    >
                      {pastedUrlSuccess ? (
                        <Check size={14} className="text-emerald-500 dark:text-emerald-400" />
                      ) : (
                        <ClipboardPaste size={14} className="text-indigo-600 dark:text-indigo-400" />
                      )}
                    ...
                    <button
                      ...
                      disabled={isParsing}
                      className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-semibold text-xs rounded-xl transition-colors flex items-center justify-center space-x-2 shrink-0 cursor-pointer shadow-md shadow-indigo-600/20"
                    >
                      <GitBranch size={14} />
          ...
          {activeTab === 'zip' && (
            <div className="relative border-2 border-dashed border-slate-200 dark:border-zinc-800 hover:border-indigo-500/50 rounded-2xl p-8 bg-slate-50 dark:bg-zinc-900/40 text-center transition-all group">
              <input
                type="file"
                accept=".zip"
              ...
              />
              <div className="w-10 h-10 rounded-xl bg-indigo-600/10 border border-indigo-500/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center mx-auto mb-3 group-hover:scale-105 transition-transform">
                <Upload size={20} />
              </div>
              <h3 className="font-semibold text-slate-900 dark:text-zinc-100 text-xs">
          ...
          {activeTab === 'files' && (
            <div className="relative border-2 border-dashed border-slate-200 dark:border-zinc-800 hover:border-indigo-500/50 rounded-2xl p-8 bg-slate-50 dark:bg-zinc-900/40 text-center transition-all group">
              ...
              <p className="text-[11px] text-slate-500 dark:text-zinc-500 mt-1">Click to open system file chooser</p>
            </div>
          )}

          {activeTab === 'paste' && (
            <div className="bg-white dark:bg-zinc-900/80 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 space-y-3 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center space-x-2">
                  <label className="text-xs font-semibold text-slate-700 dark:text-zinc-300 font-mono">
                    File Name:
                  </label>
                  <input
                    type="text"
                    ...
                    className="bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-lg px-2.5 py-1 text-xs text-slate-900 dark:text-zinc-200 font-mono focus:outline-none focus:border-indigo-500"
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 active:bg-slate-300 text-slate-700 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-white rounded-lg text-xs font-mono flex items-center space-x-1.5 transition-colors border border-slate-200 dark:border-zinc-700 cursor-pointer"
                >
                  {pastedSnippetSuccess ? (
                    <Check size={13} className="text-emerald-500 dark:text-emerald-400" />
                  ) : (
                    <ClipboardPaste size={13} className="text-indigo-600 dark:text-indigo-400" />
                  )}
              ...
              <textarea
                ...
                rows={10}
                className="w-full bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl p-3 font-mono text-xs text-slate-900 dark:text-zinc-200 placeholder-slate-400 dark:placeholder-zinc-600 focus:outline-none focus:border-indigo-500"
              />

              <button
                onClick={handleStagePaste}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-medium text-xs rounded-xl transition-colors flex items-center space-x-2 cursor-pointer shadow-md shadow-indigo-600/20"
              >
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-zinc-800/80">
            ...
            <div className="flex flex-wrap gap-1.5 text-xs font-mono text-slate-500 dark:text-zinc-400">
              {['JavaScript', 'TypeScript', 'Python', 'Java', 'Go', 'Rust', 'C++', 'C#', 'PHP', 'Ruby', 'Kotlin', 'SQL'].map((lang) => (
                <span key={lang} className="px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-slate-700 dark:text-zinc-300 text-[11px]">
                  {lang}
                </span>
              ))}
            </div>
        ...
        <div className="bg-white dark:bg-zinc-900/90 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 space-y-5 shadow-xl text-slate-800 dark:text-zinc-100">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-zinc-800">
            <div>
              <div className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest font-mono">
                Staged Repository
              </div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mt-0.5">{stagedProject.name}</h3>
            </div>
            <button
              onClick={() => setStagedProject(null)}
              className="p-1.5 rounded-lg text-slate-400 hover:text-slate-900 dark:text-zinc-400 dark:hover:text-white bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 transition-colors"
              title="Reset Upload"
            >
              <X size={16} />
            </button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800/80 p-3 rounded-xl">
              <div className="text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase font-mono">Total Files</div>
              <div className="text-xl font-bold text-slate-900 dark:text-white mt-0.5 font-mono">{stagedProject.files.length}</div>
            </div>

            <div className="bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800/80 p-3 rounded-xl">
              <div className="text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase font-mono">Lines of Code</div>
            ...
            <div className="bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800/80 p-3 rounded-xl">
              <div className="text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase font-mono">Primary Language</div>
            ...
            <div className="bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800/80 p-3 rounded-xl">
              <div className="text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase font-mono">Config Files</div>
              <div className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 mt-1.5 font-mono">
          ...
          {/* File List Preview */}
          <div className="bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800/80 rounded-xl p-3">
            <div className="text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider mb-2 font-mono">
              Detected Files Preview ({stagedProject.files.length})
            </div>
            <div className="max-h-36 overflow-y-auto space-y-1 divide-y divide-slate-100 dark:divide-zinc-900 font-mono text-xs text-slate-700 dark:text-zinc-300">
              {stagedProject.files.map((file) => (
                <div key={file.path} className="pt-1 flex items-center justify-between">
                  <span className="truncate">{file.path}</span>
                  <span className="text-slate-400 dark:text-zinc-500 text-[10px]">{file.lines} lines</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {isParsing && (
        <div className="p-3.5 rounded-xl bg-slate-100 dark:bg-zinc-900 border border-indigo-500/30 flex items-center space-x-3 text-xs text-indigo-700 dark:text-indigo-300 font-mono">
      ...
      {parseError && (
        <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 flex items-center space-x-3 text-xs text-rose-700 dark:text-rose-300 font-mono">
